...
mod parser {
  ...
  fn applications() {
    ...
    parse_eq!("map (|x| x) [1]", app(app(var("map"), lam("x", var("x"))), arr(&[num(1.0)])));
    parse_eq!("|x| map (λy → [get 0 y, 2]) x", lam("x", app(app(var("map"), lam("y", arr(&[app(app(var("get"), num(0.0)), var("y")), num(2.0)]))), var("x"))));
    parse_eq!("(get 0) [0]", app(app(var("get"), num(0.0)), arr(&[num(0.0)])));
    parse_eq!("   ( ( ((( get 0 ))   )) [0]   )  ", app(app(var("get"), num(0.0)), arr(&[num(0.0)])));
    parse_eq!("((((get  0)))) [ 0 ,1]", app(app(var("get"), num(0.0)), arr(&[num(0.0), num(1.0)])));
  }
  ...
  fn higher_order_application() {
    parse_eq!("\\x -> \\y -> x y 2", lam("x", lam("y", app(app(var("x"), var("y")), num(2.0)))))
  }

  #[test]
  #[allow(non_snake_case)]
  fn parses_S_combinator() {
    parse_eq!("λf -> λg → \\x → f x (g x)", lam("f", lam("g", lam("x", app(app(var("f"), var("x")), app(var("g"), var("x")))))))}

  #[test]
  fn objects_and_arrays() {
    parse_eq!("|f| f { \"name\": { \"first\": 42, \"second\": [ 10, null ] } } 5", lam("f", app(app(var("f"), obj(&[("name", obj(&[("second", arr(&[num(10.0), Const(Null)])), ("first", num(42.0)),]))])), num(5.0))))
  }

  #[test]
  fn dot_patterns() {
    parse_eq!("\\x -> x.1.a.\"b\".c.4", lam("x", app(app(var("get"), num(4.0)), app(app(var("get"), expr_str("c")), app(app(var("get"), expr_str("b")), app(app(var("get"), expr_str("a")), app(app(var("get"), num(1.0)), var("x"))))))))
  }

  #[test]
  fn pipes() {
    parse_eq!("get 0 | get \"phones\" | get 0", lam("x", app(app(var("get"), num(0.0)), app(lam("x", app(app(var("get"), expr_str("phones")), app(app(var("get"), num(0.0)), var("x")))), var("x")))));
  ...
  fn parse_if_then_else() {
    parse_eq!("if null then 2 else 5", if_then_else(Const(Null), num(2.0), num(5.0)));
